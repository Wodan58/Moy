...
can be seen by executing `ccmake .`. This program reads the latest
...
reduce the functionality in the main function to the bare minimu. The only
...
is not recursive but still fails because `linrec` recurses.
