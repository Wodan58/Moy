...
{ "concat_str", "S T  ->  U", "Sequence U is the concatenation of sequences S and T.\n" },
...
{ "cons_str", "X A  ->  B", "Aggregate B is A with a new member X (first member for sequences).\n" },
...
{ "enconcat_str", "X S T  ->  U", "Sequence U is the concatenation of sequences S and T\nwith X inserted between S and T (== swapd cons concat).\n" },
{ "equal", "T U  ->  B", "(Recursively) tests whether trees T and U are identical.\n" },
{ "exp", "F  ->  G", "G is e (2.718281828...) raised to the Fth power.\n" },
{ "fabs", "N1  ->  N2", "Integer N2 is the absolute value (0,1,2..) of integer N1,\nor float N2 is the absolute value (0.0 ..) of float N1.\n" },
...
{ "has_set", "A X  ->  B", "Tests whether aggregate A has X as a member.\n" },
...
{ "put_bool", "X  ->", "Writes X to output, pops X off stack.\n" },
{ "put_dbl", "X  ->", "Writes X to output, pops X off stack.\n" },
{ "put_int", "X  ->", "Writes X to output, pops X off stack.\n" },
{ "put_str", "X  ->", "Writes X to output, pops X off stack.\n" },
...
{ "small_int", "X  ->  B", "Tests whether aggregate X has 0 or 1 members, or numeric 0 or 1.\n" },
{ "small_lst", "X  ->  B", "Tests whether aggregate X has 0 or 1 members, or numeric 0 or 1.\n" },
{ "small_str", "X  ->  B", "Tests whether aggregate X has 0 or 1 members, or numeric 0 or 1.\n" },
...
{ "split_str", "A [B]  ->  A1 A2", "Uses test B to split aggregate A into sametype aggregates A1 and A2.\n" },
...
{ "step_str", "A [P]  ->  ...", "Sequentially putting members of aggregate A onto stack,\nexecutes P for each member of A.\n" },
...
{ "swons_set", "A X  ->  B", "Aggregate B is A with a new member X (first member for sequences).\n" },
{ "swons_str", "A X  ->  B", "Aggregate B is A with a new member X (first member for sequences).\n" },
...
{ "uncons_str", "A  ->  F R", "F and R are the first and the rest of non-empty aggregate A.\n" },
